...
It includes:
- `LoginViewSet`: Handles user authentication, including login and session management.
...
- `POST /login/`:
    ...
    - Logs them in upon successful authentication.
"""
...
class ForgotPasswordViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                user_otp = Otp.objects.get(token=token, code=code)
